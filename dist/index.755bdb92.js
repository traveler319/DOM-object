window.dom = {
    ...
    empty (node) {
        const { childNodes  } = node; // 新语法，等价于const childNodes = node.childNodes
        const array = [];
    ...
    text (node, string) {
        if (arguments.length === 2) {
            ...
            else node.textContent = string; // Chrome / Firefox
    ...
    style (node, name, value) {
        ...
        node.style[name] = value;
            ...
            for(let key in name)/**
           * key可能是border也可能是color
           * node.style.border = ...
           * node.style.color = ...
           * 用node.style.key的形式会变成字符串，key是变量只能放在[]内
